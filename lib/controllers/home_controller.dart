...
class HomeController extends GetxController {
  ...
  void _loadChats() {
    ...
    if (currentUserId != null) {
      ...
      // Listen for chat updates and force UI refresh
      ever(_allChats, (_) {
        if (_isSearching.value && _searchQuery.value.isNotEmpty) {
          _performSearch(_searchQuery.value);
        }
        // Force update to ensure UI reflects latest changes
        update();
      });
  ...
  void _performSearch(String query) {
    ...
    _filteredChats.value =
        _allChats.where((chat) {
          ...
          final emailMatch =
              otherUser.email.toLowerCase().contains(lowercaseQuery);
  ...
  void _sortSearchResults(String query) {
    filteredChats.sort((a, b) {
      ...
      final exactMatchA =
          userA.displayName.toLowerCase().startsWith(query);
      final exactMatchB =
          userB.displayName.toLowerCase().startsWith(query);
  ...
  void openChat(ChatModel chat) {
    final otherUser = getOtherUser(chat);
    if (otherUser != null) {
      Get.toNamed(AppRoutes.chat, arguments: {'chatId': chat.id, 'otherUser': otherUser});
    }
  }
  ...
  Future<void> refreshChats() async {
    ...
    try {
      ...
      _error.value = 'Failed to refresh chats: ${e.toString()}';
      print(e.toString());
    } finally {
